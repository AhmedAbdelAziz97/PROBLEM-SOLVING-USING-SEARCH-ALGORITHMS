 ...
 */
public class Greedy implements AI_Interface {   
    ...
    public void Search( ArrayList <Link> links,char start,char goal)
    {
        ...
        for(int i=0;i<unExplored.size();i++)
        {
            for(int x=0;x<heuristic.size();x++)
            {
                ...
                {
                    unExplored.get(i).setCost(heuristic.get(x).getCost());
                    break;
                }
            }
        }
            ...
            for (int i=1;i<frontier.list.size();i++)
            {
                ...
                if(Explored.list.get(Explored.list.size()-1).isGoal(goal)==true)
                {
                    ...
                    for(int j=1;j<Path.size();j++)
                    {
                        ...
                        {
                            if((Path.get(j).getName()==links.get(x).getC2())&&(Path.get(j).getParent().getName()==links.get(x).getC1()))
                            {
                                Path.get(j).setCost(links.get(x).getLinkCost());
                            }
                        }
                    }
                    Result.list=Path;
